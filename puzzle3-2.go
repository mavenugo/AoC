...
func main() {
	...
	for x := 0; x < ax; x++ {
		for y := 0; y < ay; y++ {
			if path[x][y].X == 3 {
				xd := path[x][y].steps[0] + path[x][y].steps[1]
				fmt.Printf("I : {%d, %d}, X:{%d, %d}, M:%d\n", startx, starty, x, y, xd)
				// fmt.Printf("Intersections : {%d, %d}\n", x, y)
				if xd < md {
					md = xd
					//fmt.Printf("changed md := %d\n", md)
				}
			}
		}
	}
...
func alloc(wire []string) (int, int, int, int) {
	L := 0
	R := 0
	U := 0
	D := 0
	...
	return R, abs(L), D, abs(U)
}
func trace(wire []string, wn int, x int, y int) {
	step := 0
	for _, pos := range wire {
		dir := strings.SplitN(pos, "", 2)
		l, _ := strconv.Atoi(dir[1])
		// fmt.Printf(" [[%d : (%d, %d)]] ", l, x, y)
		switch dir[0] {
		case "R":
			end := x + l
			for ; x < end; x++ {
				if path[x][y].X == 0 {
					path[x][y].X = wn
					path[x][y].steps[wn-1] = step
				} else if path[x][y].X != wn {
					path[x][y].X = 3
					path[x][y].steps[wn-1] = step
				}
				step++
